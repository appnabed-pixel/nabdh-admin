...
export default function Sidebar() {
  ...
  return (
    <div style={{ width: 240, minWidth: 240, background: '#fff', borderLeft: '1px solid #E8EDF2', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0, fontFamily: 'Cairo, sans-serif' }} dir="rtl">

      {/* Logo */}
      <div style={{ padding: '16px 18px', borderBottom: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', gap: 10, height: 56, minHeight: 56 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 10px' }}>
        {groups.map((g, gi) => (
          <div key={gi} style={{ marginBottom: gi < groups.length - 1 ? 20 : 0 }}>
            {g.label && (
              <p style={{ fontSize: 10, fontWeight: 700, color: '#CBD5E1', padding: '0 8px', marginBottom: 4, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                {g.label}
              </p>
            )}
            {g.items.map(item => {
              ...
              return (
                <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }}>
                  <div style={{
                    ...
                    {/* Icon */}
                    <Icon size={15} color={active ? '#14B8A6' : '#94A3B8'} strokeWidth={active ? 2.5 : 2} style={{ flexShrink: 0 }} />
                    {/* Label */}
                    ...
                    {/* Badge */}
                    {badge ? (
                      <span style={{ fontSize: 10, fontWeight: 700, background: '#FEF2F2', color: '#EF4444', padding: '1px 6px', borderRadius: 5, lineHeight: 1.6 }}>
                        {badge}
                      </span>
                    ) : null}
                  </div>
